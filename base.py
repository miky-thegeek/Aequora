...
def getCardTransaction(csvCard, dateBank, descPartsBank):
    ...
    for lineCard in csvCard.itertuples():
        ...
        if dateBank == lineCard[1]:
            ...
            #timeCard = lineCard[2].to_pydatetime()
...
#def getPayPalTransaction(csvPayPal)
    ...
    if "MASTERCARD" in lineBank[3]:
        ...
        for linePayPal in csvFilePayPal.itertuples():
            ...
            for daysNumber in [2, 3, 1, 0]:
                if abs(float(lineBank[4].replace(',', '.'))) == abs(linePayPal[6]) and compute_next_business_day.next_number_business_day(linePayPal[1].to_pydatetime(), 'IT', daysNumber) == lineBank[2].to_pydatetime():
                    print(linePayPal)
